...
public class UnitGenerator {
    ...
    public static Unit createUnit(String[] stats)
    {
        ...
        int price = parseIntSafe(stats, 3);  //Displays the Unit's Price
        int armor = parseIntSafe(stats, 4);  // Displays the Units Armor Level
        int health = parseIntSafe(stats, 5);  //Displays the Units Health
        int sightRange = parseIntSafe(stats, 6);  //Displays the Unit's sight range
        double unseenRange = parseDoubleSafe(stats, 7, 0.0); //Displays the unseen Range
        int speed = parseIntSafe(stats, 8);  //Displays the unit's speed
        int weight = parseIntSafe(stats, 9);  //Displays its weights
        String abilities = stats.length > 10 ? stats[10] : ""; //Lists its abilities
        int extra = parseIntSafe(stats, 11); //List the Extra stats that are associated  with its unit type

            //TODO: use a switch statement instead of if-else to create a new unit object
        ...
        // Based on the unit type (category), create the appropriate unit object.
            ...
            private static int parseIntSafe(String[] stats, int index)
            {
